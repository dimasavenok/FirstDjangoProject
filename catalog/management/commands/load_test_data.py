...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not superusers.exists():
            self.stdout.write(self.style.WARNING("Суперпользователи не найдены"))
        else:
            for su in superusers:
                su.groups.add(moderator_group, content_manager_group)
                self.stdout.write(self.style.SUCCESS(
                    f"Суперпользователь {su.email} добавлен в группы 'Модератор продуктов' и 'Контент-менеджер'"
                ))